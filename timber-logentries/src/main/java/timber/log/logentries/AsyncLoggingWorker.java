...
public class AsyncLoggingWorker {
    ...
    private static final int RECONNECT_WAIT = 500; // milliseconds.
    ...
    public AsyncLoggingWorker(Context context, String logToken, Config config) throws IOException {
        if (config == null) {
            config = new Config();
        }
        ...
        appender = new SocketAppender(logToken, config, this.sendRawLogMessage);
        appender.start();
        started = true;
    }
    ...
    private void tryOfferToQueue(String line) {
//        if (queue.size() >= QUEUE_SIZE) {
//            queue.offer("Log Queue is full");
//        } else {
//            queue.offer(line);
//        }
        ...
        if (!queue.offer(line)) {
            ...
            the local storage - the we have three problems:
            1) Correct joining of logs from the queue and from the local storage (and we need some
            right event to trigger this joining);
            2) Correct order of logs after joining;
            3) Data consistence problem, because we're
    ...
    private class SocketAppender extends Thread {
        ...
        public SocketAppender(String token, Config config, boolean sendRawLogMessage) {
            super("Logentries Socket appender");

            // Don't block shut down
            setDaemon(true);

            this.useHttpPost = config.useHttpPost;
            this.useSsl = config.useSsl;
            this.isUsingDataHub = config.isUsingDataHub;
            this.dataHubAddr = config.dataHubAddr;
            this.dataHubPort = config.dataHubPort;
            this.token = token;
            this.logHostName = config.logHostName;
            this.sendRawLogMessage = sendRawLogMessage;
        }
        ...
        private boolean reopenConnection(int maxReConnectAttempts) throws InterruptedException, InstantiationException {
            ...
            for (int attempt = 0; attempt < maxReConnectAttempts; ++attempt) {
                try {
                    ...
                    // Ignore the exception and go for the next iteration.
        ...
        private boolean tryUploadSavedLogs() {
            ...
            try {
                ...
                try {
                    ...
                    Log.e(TAG, "Cannot save logs to the local storage - part of messages will be dropped! Error: " + ioEx2.getMessage());
                }
            }
        ...
        public void run() {
            try {
                        ...
                        try {
                            ...
                            if (numFailures >= MAX_NETWORK_FAILURES_ALLOWED) {
                                ...
                                try {
                                    ...
                                    Log.e(TAG, "Cannot save the log message to the local storage! Error: " + ex.getMessage());
                                }
                ...
                try {
                    ...
                    Log.e(TAG, "Cannot save logs queue to the local storage - all log messages will be dropped! Error: " + e.getMessage());
                }
            }
    ...
    class Config {
        ...
        public void setUsingDataHub(boolean usingDataHub) {
            isUsingDataHub = usingDataHub;
        }
